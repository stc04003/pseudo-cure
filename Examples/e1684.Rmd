...
```{R, error = TRUE}
smcure(Surv(FAILTIME, FAILCENS) ~ AGE + TRT + SEX, ~ AGE + TRT + SEX,
...
coxPHMC <- smcure(Surv(FAILTIME, FAILCENS) ~ AGE + TRT + SEX + TRT.SEX, ~ AGE + TRT + SEX + TRT.SEX, 
                  data = e1684, model = "ph", nboot = 100)
```
...
```{R}
e1684.inc$resid <- e1684.inc$curei - 1 / (exp(-model.matrix(~ AGE + TRT * SEX, e1684.inc) %*% fit.inc$beta) + 1)
